...
# Import data
library(readxl)
newfinal_monthly_all <- read_excel("C:/Users/kschmitt1/Knipp Wolf NLG Dropbox/Knipp Group Team Folder/5 - STAFF/3 - STAFF/KSchmitt/Research Reports/Interest Rates (General Report 1)/newfinal_monthly_all.xlsx", 
    col_types = c("date", "numeric", "numeric", 
        "numeric", "numeric", "numeric"))
# Define variables
CapRate <- ts(newfinal_monthly_all$cap, start = c(2013, 4), end = c(2024, 3), frequency = 12)
DGS10 <- ts(newfinal_monthly_all$DGS10, start = c(2013, 4), end = c(2024, 3), frequency = 12)
DFF <- ts(newfinal_monthly_all$DFF, start = c(2013, 4), end = c(2024, 3), frequency = 12)
lnDGS10 <- ts(newfinal_monthly_all$lnDGS10, start = c(2013, 4), end = c(2024, 3), frequency = 12)
lnDFF <- ts(newfinal_monthly_all$lnDFF, start = c(2013, 4), end = c(2024, 3), frequency = 12)
newfinal_monthly_all$observation_date <- as.Date(newfinal_monthly_all$observation_date)
...
```         
should this set have the "n" prefix for new, or should this have no prefix and I make "new" later?
```
